...
var animal_names = []
var animal_database = null

func _ready():
	# Load the animal database script only once
...
func spawn_random_animal():
	# Ensure we have an animal database loaded and a list of animal names
	if animal_database == null or animal_names.size() == 0:
		print("Failed to load animal data!")
		return
